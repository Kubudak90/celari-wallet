  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 md:px-12 py-5 flex items-center justify-between bg-bg/90 backdrop-blur-xl border-b border-border-default">
      ...
      <a href="#" className="flex items-center gap-3">
        <Logo size="sm" />
        <span className="font-display text-lg tracking-[8px] uppercase text-text-warm font-light">
          Celari
        </span>
      </a>
      ...
      <div className="hidden md:flex items-center gap-8">
        ...
        <Link
          href="/bridge"
          className="font-mono text-[10px] tracking-[3px] uppercase text-copper hover:text-copper-light transition-colors duration-300"
        >
          {t("bridge")}
        </Link>
        <LanguageSwitcher />
        <a
          href="#waitlist"
          className="font-mono text-[10px] tracking-[3px] uppercase text-bg bg-burgundy px-6 py-2.5 rounded-md hover:bg-burgundy-light transition-all duration-300"
        >
          {t("earlyAccess")}
        </a>
      </div>
      ...
      {mobileOpen && (
        <div className="absolute top-full left-0 right-0 bg-bg/95 backdrop-blur-xl border-b border-border-default p-6 flex flex-col gap-4 md:hidden">
          ...
          <Link
            href="/bridge"
            onClick={() => setMobileOpen(false)}
            className="font-mono text-[10px] tracking-[3px] uppercase text-copper hover:text-copper-light transition-colors"
          >
            {t("bridge")}
          </Link>
          <LanguageSwitcher />
          <a
            href="#waitlist"
            onClick={() => setMobileOpen(false)}
            className="font-mono text-[10px] tracking-[3px] uppercase text-bg bg-burgundy px-6 py-2.5 rounded-md text-center"
          >
            {t("earlyAccess")}
          </a>
        </div>
