export function DiamondSeparator({ className }: { className?: string }) {
  return (
    <div className={`flex items-center justify-center gap-3 ${className ?? ""}`}>
      <div className="w-10 h-px bg-gradient-to-r from-transparent to-border-warm" />
      <div className="w-[5px] h-[5px] bg-copper rotate-45" />
      <div className="w-10 h-px bg-gradient-to-l from-transparent to-border-warm" />
    </div>
  );
}
...
export function SectionHeader({
  ...
  return (
    <div className={`text-center mb-20 ${className ?? ""}`}>
      ...
      <h2 className="font-display text-[clamp(28px,4vw,44px)] text-text-warm tracking-[6px] uppercase font-light">
        {title}
      </h2>
...
export function GeoBars({ className }: { className?: string }) {
  ...
  return (
    <div className={`flex justify-center gap-[3px] opacity-[0.12] ${className ?? ""}`}>
      {heights.map((h, i) => (
        <div key={i} className="w-[2px] bg-copper rounded-full" style={{ height: h }} />
      ))}
    </div>
  );
}
