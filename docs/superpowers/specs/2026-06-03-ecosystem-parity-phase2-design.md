...
- **Maximal reuse:** route the RPC method through the SAME machinery the wallet-sdk channel already uses for the identical method, differing only in the response transport. Build a wallet-sdk-style decrypted message `{ type: bare, args: params, messageId }` and feed it into the existing flow; make the shared responders branch on `session.kind`.
- `classifySecureMessage({ method: bare, locked: await _bgIsLocked() })`:
  - **forward** (unlocked read): call `_wsForwardToPxe(decrypted, session, sessionId)` — the existing helper that does `PXE_WALLET_METHOD` → `handleWalletMethod` → encrypt + send. Make `_wsForwardToPxe` (and `_wsSendSignError`) send via `_providerRespond` when `session.kind === "provider"`, else the existing wallet-sdk `secure-response`. The provider request's outer `requestId` is carried so `_providerRespond` echoes it.
  - **sign-popup** (write: `sendTx`/`createAuthWit`): reuse the wallet-sdk **wssign** popup + `_wsPendingSignRequests` (it already renders + executes wallet-sdk writes correctly). Park the request tagged with the provider session; on `WS_APPROVE_SIGN`, `_wsForwardToPxe` runs and (per the `kind` branch above) responds over the provider channel; on `WS_REJECT_SIGN`, the error routes the same way.
  - **unlock-then-resume** (locked read): reuse `_wsQueueLockedRead(decrypted, session, sessionId)`; on unlock, `_wsDrainLockedReads` replays via `_wsForwardToPxe` → provider response.
- The bespoke method cases (`DAPP_*`, `GET_*`, `GET_WITHDRAW_PROOF`) stay as-is. Net new background code is mostly the `RPC` dispatch + the `session.kind` response branch in the shared responders.
