...
- [ ] **iOS Rebrand**
  ...
  - [x] Phase B — V3 Screens (plan: `2026-04-25-ios-v3-screens.md`, branch: `feat/ios-v3-screens`) — 8 reusable components (BalanceHeader / QuickActionButton / AccountCard / AssetRow / TransactionRow / ScreenHeader / PercentChangePill / AmountInput) and 9 view files (HomeViewV3 / SendViewV3 / ReceiveViewV3 / AssetsViewV3 / ActivityViewV3 / DiscoverViewV3 / OnboardingViewV3 / LoadingViewV3 + SettingsViewV3 expanded). RootViewV3 wires Home tab to a `NavigationStack` with `HomeRoute` enum so Send / Receive push from quick actions; Swap and Buy remain `ComingSoonPlaceholder` until on-chain support lands. Loading and Onboarding now use V3 views. Restore / Recovery / Backup / AddAccount / GuardianSetup still route to V2 views — rebuilt in a separate "wallet management flows" plan. SendViewV3 Review CTA is intentionally a stub for Phase B+ wiring to `PXEBridge.transfer`. Build green; simulator screenshots captured at `branding/exports/ios-screenshots/phase-b/`. V1+V2 view trees intact.
  - [x] Phase C — V1 + V2 cleanup partial (branch: `feat/ios-v3-cleanup`). Deleted V1 dashboard / send / receive / settings / restore / backup / add-account / onboarding / loading / recovery view trees; deleted V2 home / send / receive / history / swap / bridge / fee-juice / profile / shield / prover-benchmark views and the V2 components only those used (AccountSelectorV2 / ActionButtonV2 / BalanceCardV2 / TabBarV2 / TokenRowV2). Kept V1 modal views (Confirm / WcApprove / WalletConnect / AddToken / AddNftContract / NftDetail) and V2 wallet-management views (Onboarding / Restore / RecoverAccount / GuardianSetup / Backup / AddAccount) until V3 rebuilds land. RootViewV3 now routes the kept V1 modals so they display when reached. Build green; ~6500 lines removed.
  - [x] Phase D — Wallet management V3 (branch: `feat/ios-v3-wallet-mgmt`) — AddAccountViewV3 / BackupViewV3 / RestoreViewV3 / GuardianSetupViewV3 / RecoverAccountViewV3 ported off V2 with full logic + biometric gating preserved. ScreenHeaderRouted component added for store.screen-routed flows. V2/Views directory and NetworkPickerSheet deleted; V2/Theme + V2/Components/PXELogViewV2 retained until Phase E.
  - [x] Phase E — V1 modal V3 rebuild + full V1+V2 deletion (branch: `feat/ios-v3-modals`) — ConfirmTxViewV3 / WcApproveViewV3 / WalletConnectViewV3 / AddTokenViewV3 / AddNftContractViewV3 / NftDetailViewV3 ported off V1, all biometric gating + audit fixes preserved. PXELogViewV2 → V3/Components/PXELogView. Color(hex2:) moved into V3Theme. V1 + V2 directories deleted entirely. iOS structure: Assets.xcassets / Core / Models / Resources / V3. Build green.
- [x] **Website Rebrand** (foundation; branch: `feat/website-rebrand`) — globals.css `@theme inline` switched from navy + teal to V3 gold-on-dark palette + light-mode override. Logo.tsx rewritten to use `/logo-mark.svg` and `/logo-lockup.svg` vector assets with mark/lockup variants. Hero.tsx rebuilt to match reference image 3: split-color "Your crypto. / Your privacy. / Always." headline, Apple/Google download CTAs, gold privacy shield badge, iPhone mockup with embedded lockup + balance preview + 4 quick-action circles, 4-ring gold radial backdrop. `next/font` adds Outfit alongside Inter. Brand assets (logo SVGs, favicon, og-dark) copied to `website/public/`. i18n keys extended in en + tr. `website/` gitignore rule replaced with targeted ignores for `.next/`, `node_modules/`, `out/`, `.env*.local`, `next-env.d.ts`, `.vercel/`. Build green (6 static pages). Deployed to Vercel: production at `https://celari-wallet.vercel.app`. Header / Features / Footer / HowItWorks / Roadmap / AztecSection / WaitlistCTA / bridge views still use existing layouts but inherit the new gold palette automatically through the @theme update — a follow-up plan can rebuild those component layouts to match reference image 3.
- [x] **Extension Rebrand** (branches: `feat/extension-rebrand` foundation + `feat/extension-v3-full` finish) — V2 Aztec palette swapped for V3 gold-on-dark in popup.css `:root` (legacy variable names kept as aliases so the ~1000-line popup.css and 3000-line popup.js compile unchanged). Added direct gold tokens (`--gold-primary`/`--soft`/`--glow`/`--deep`) and a `[data-theme="light"]` override. Auto-generated `tokens.css` now linked from popup.html + sidepanel.html so token-pipeline updates flow into the extension. Outfit-Light.ttf bundled in `fonts/`, registered as `@font-face 'Outfit'`; mini lockup replaces icon+text in the header so the wordmark renders in Outfit Light. Logo references swapped from PNG icons to `icons/logo-mark.svg` / `icons/logo-lockup.svg`; SVG variants copied into `extension/public/icons/`. Theme toggle implemented end-to-end: `applyTheme(pref)` + `setTheme(pref)` JS helpers persist preference to `chrome.storage.local["celari_theme"]`, follow `prefers-color-scheme` for `"system"` mode, re-apply live on system change; init() loads pref before first render to prevent flash; Settings has a 3-pill Appearance row (System / Dark / Light) with gold-gradient active state. Quick action row rebuilt to gold-bordered 44px circles per reference image 2 with hover glow + press scale. Balance card text colors moved from hard-coded `#FFFFFF`/`rgba(255,255,255,…)` to `var(--text-warm)` / `var(--text-body)` so it adapts to both themes. manifest.json `icons` + `default_icon` extended with 32px entry. popup.js inline color refs (SVG icon strokes, "C" character, claim button text) tokenized. `node extension/build.mjs` produces a clean `extension/dist/` (production).
