...
- [x] **Extension Rebrand** (branch: `feat/extension-rebrand`) — popup.css `:root` block rewritten: V2 Aztec navy + orange palette swapped for V3 gold-on-dark. Legacy variable names (`--burgundy`, `--aztec-dark`, `--copper`, `--bronze`) retained as aliases pointing at gold values so the ~1000-line popup.css and 3000-line popup.js compile unchanged. Added direct gold tokens (`--gold-primary`/`--soft`/`--glow`/`--deep`) and a `[data-theme="light"]` override for system theme. popup.js inline color references updated (SVG icon strokes, "C" character, claim button text contrast). Balance card hard-coded navy gradient → bg-card / bg-elevated for V3 chrome. manifest.json `icons` + `default_icon` extended with 32px entry. `node extension/build.mjs` produces clean `extension/dist/` (production). No layout / DOM / behaviour changes — pure palette swap.
