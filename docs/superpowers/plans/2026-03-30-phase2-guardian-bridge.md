...
## Task 7: Bridge L2 v4.2.1 Compatibility (DONE)

**Status:** Completed 2026-05-14. Latest stable aztec-packages is `v4.2.1` (v4.3.0 is still nightly), so we target v4.2.1 — the same tag as `contracts/celari_passkey_account` and `contracts/celari_recoverable_account`.

**Files touched:**
- `bridge/contracts/l2/bridged_token/Nargo.toml` (tag bumped)
- `bridge/contracts/l2/celari_token_bridge/Nargo.toml` (tag bumped)
- `scripts/compile-contracts.sh` (extended to compile + transpile + strip bridge artifacts in the same pipeline as the account contracts; also fixed a Step-3 bug where `node -e` argv handling silently skipped strip on all artifacts)

**No Noir source changes were needed** — every API the bridge uses (`consume_l1_to_l2_message`, `message_portal`, `maybe_msg_sender`, `#[authorize_once]`, `#[external("public"/"private")]`) is unchanged between v4.1.2 and v4.2.1.

**Compatibility evidence:**
- New `bridged_token-BridgedToken.json` and `celari_token_bridge-CelariTokenBridge.json` no longer contain `utilityFetchTaggedLogs` / `fetchTaggedLogs` oracle calls. The `.bak` (v4.1.2) artifacts did — which was the exact PXE-incompatibility described in commit `4e742f0`.
- All 4 contracts produced AVM-transpiled bytecode + verification keys via `bb aztec_process` on `aztecprotocol/aztec:4.2.1`.
...
- [x] **Step 1: Bump Nargo tags to v4.2.1** — done
- [x] **Step 2: Verify source-level API compatibility** — confirmed (compile passed; oracle surface matches v4.2.x)
- [x] **Step 3: Run full pipeline (nargo compile + bb aztec_process + strip prefixes)** — all 4 artifacts produced

**Commit (to land):**

```bash
git add bridge/contracts/l2/bridged_token/Nargo.toml \
        bridge/contracts/l2/celari_token_bridge/Nargo.toml \
        scripts/compile-contracts.sh \
        bridge/contracts/l2/bridged_token/target/bridged_token-BridgedToken.json \
        bridge/contracts/l2/celari_token_bridge/target/celari_token_bridge-CelariTokenBridge.json \
        contracts/celari_passkey_account/target/celari_passkey_account-CelariPasskeyAccount.json \
        contracts/celari_recoverable_account/target/celari_recoverable_account-CelariRecoverableAccount.json \
        docs/superpowers/plans/2026-03-30-phase2-guardian-bridge.md

git commit -m "feat(bridge): align L2 bridge with aztec-nr v4.2.1

- Bump bridge/contracts/l2/{bridged_token,celari_token_bridge}/Nargo.toml
  from v4.1.2 → v4.2.1 (matches account contracts, latest stable).
- Extend scripts/compile-contracts.sh to include both bridge contracts
  in the compile + bb aztec_process + prefix-strip pipeline.
- Fix Step-3 strip: node -e omits [eval] from argv, so the previous
  slice(1) over (\${BB_ARGS[@]/-i/}) silently skipped every file when
  combined with 2>/dev/null. Rebuilt with explicit PATHS array.
...
| 7 | 2 | Bridge L2 v4.2.1 compat (DONE 2026-05-14) | Independent |
