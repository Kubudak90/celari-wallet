...
// Re-export chonk types for consumers
pub use barretenberg_rs::generated_types::{
    ChonkProveResponse, ChonkComputeVkResponse, ChonkVerifyResponse,
    ChonkProof, GoblinProof,
};
...
/// Initialize the Grumpkin curve SRS (required for chonk/IVC proving).
///
/// Grumpkin SRS is needed for the recursive components of IVC proofs
/// (ECCVM and IPA sub-proofs). Fixed size: 2^16 + 1 = 65537 points.
///
/// # Arguments
/// * `g1_data` - Grumpkin G1 points data
/// * `num_points` - Number of G1 points (typically 65537)
pub fn srs_init_grumpkin(
    g1_data: &[u8],
    num_points: u32,
) -> Result<(), String> {
    let mut api = get_api();
    api.srs_init_grumpkin_srs(g1_data, num_points)
        .map_err(|e| format!("srs_init_grumpkin failed: {}", e))?;
    Ok(())
}
