...
describe("passkey-walletdb-shim", () => {
  ...
  // The shim must NOT touch accountContracts.createStubAccount — the gas-estimate
  // must use the upstream permissive stub so EmbeddedWallet.sendTx auto-capture
  // can create inner authwits. (The reverted "Option C" overrode it and broke
  // real dApp authwit txs with "Unknown auth witness for message hash".)
  it("leaves accountContracts.createStubAccount untouched", () => {
    const origStub = jest.fn();
    ...
    installPasskeyWalletDbShim(wallet, new Map([["0xpass", { manager: {}, wallet: {} }]]));
    expect(wallet.accountContracts.createStubAccount).toBe(origStub);
  });
});
