...
function getDemoTokens() {
  return [
    ...
    { name: "Celari Token", symbol: "ZKP", balance: "5,000", value: "$150.00", icon: "🔐", color: "#7c3aed" },
  ];
}
...
async function handleCreatePasskey() {
  ...
  try {
    ...
    showToast("✅ Passkey cüzdan oluşturuldu!", "success");
  ...
  return `
    ...
    <div class="balance-card">
      ...
      <div class="balance-address">
        ...
        ${isPasskey ? '<span style="margin-left:4px;font-size:10px;color:var(--green)">👆 Passkey</span>' : '<span style="margin-left:4px;font-size:10px;color:var(--yellow)">🧪 Demo</span>'}
      </div>
    </div>
...
function bindDashboard() {
  ...
  document.getElementById("btn-bridge")?.addEventListener("click", () => showToast("🌉 Bridge — Faz 1'de gelecek", "success"));
  ...
  return `
    ...
    <div style="padding:20px 16px;text-align:center">
      ...
      <div style="width:200px;height:200px;margin:0 auto 16px;background:white;border-radius:var(--radius-md);display:flex;align-items:center;justify-content:center;position:relative">
        ...
        <div style="position:absolute;background:var(--accent);color:white;width:36px;height:36px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:800">Z</div>
      </div>

      <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">AZTEC AĞINDA ADRESİM</div>

      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px;word-break:break-all;font-family:monospace;font-size:12px;color:var(--cyan);margin-bottom:12px;text-align:left">
        ${address}
      </div>
...
function renderSettings() {
  ...
  return `
    ...
    <div style="padding:12px 16px">
      <!-- Account Section -->
      <div style="font-size:11px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;margin-top:4px">Hesap</div>
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-md);margin-bottom:16px;overflow:hidden">
        <div style="padding:12px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--border)">
          ...
          <div style="flex:1">
            <div style="font-weight:700;font-size:13px">${account?.label || "Hesap"}</div>
            <div style="font-size:11px;color:var(--text-muted)">${isPasskey ? "Passkey (P256)" : "Demo modu"}</div>
          </div>
          <span style="font-size:10px;padding:3px 8px;border-radius:6px;background:${isPasskey ? 'var(--green-glow)' : 'rgba(234,179,8,0.12)'};color:${isPasskey ? 'var(--green)' : 'var(--yellow)'};border:1px solid ${isPasskey ? 'rgba(16,185,129,0.25)' : 'rgba(234,179,8,0.25)'}">${isPasskey ? "Aktif" : "Demo"}</span>
        </div>
      ...
      <!-- Network Section -->
      <div style="font-size:11px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Ağ</div>
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-md);margin-bottom:16px;overflow:hidden">
        <div class="settings-row" id="btn-network-local" style="padding:12px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--border);cursor:pointer">
          <div class="network-dot ${store.network === 'local-network' && store.connected ? '' : 'disconnected'}" style="width:8px;height:8px;border-radius:50%;background:${store.network === 'local-network' ? 'var(--green)' : 'var(--border)'}"></div>
          <div style="flex:1">
            <div style="font-weight:600;font-size:12px">Local Sandbox</div>
            <div style="font-size:10px;color:var(--text-muted)">localhost:8080</div>
          </div>
          ${store.network === "local-network" ? `<span style="color:var(--accent)">${icons.check}</span>` : ""}
        </div>
        <div class="settings-row" id="btn-network-devnet" style="padding:12px;display:flex;align-items:center;gap:10px;cursor:pointer">
          <div class="network-dot" style="width:8px;height:8px;border-radius:50%;background:${store.network === 'devnet' ? 'var(--cyan)' : 'var(--border)'}"></div>
          <div style="flex:1">
            <div style="font-weight:600;font-size:12px">Aztec Devnet</div>
            <div style="font-size:10px;color:var(--text-muted)">devnet.aztec-labs.com</div>
          </div>
      ...
      <!-- Security Section -->
      <div style="font-size:11px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Güvenlik</div>
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-md);margin-bottom:16px;overflow:hidden">
        <div style="padding:12px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--border)">
          <span style="font-size:16px">🔐</span>
          <div style="flex:1">
            <div style="font-weight:600;font-size:12px">Passkey Yönetimi</div>
        ...
        <div style="padding:12px;display:flex;align-items:center;gap:10px">
          ...
          <div style="flex:1">
            <div style="font-weight:600;font-size:12px">Public Key Göster</div>
      ...
      <!-- About -->
      <div style="text-align:center;padding:8px 0;color:var(--text-muted);font-size:10px">
        Celari Wallet v0.2.0 · Aztec Network · Faz 1<br/>
        Built with 🔐 on Noir + aztec.js
      </div>
    </div>`;
}
...
function bindSettings() {
  document.getElementById("btn-back")?.addEventListener("click", () => setState({ screen: "dashboard" }));
  document.getElementById("btn-network-local")?.addEventListener("click", () => {
    setState({ network: "local-network", nodeUrl: "http://localhost:8080" });
    chrome.runtime.sendMessage({ type: "SET_NETWORK", nodeUrl: "http://localhost:8080", network: "local-network" });
    showToast("🔄 Local Sandbox'a bağlandı", "success");
  });
  document.getElementById("btn-network-devnet")?.addEventListener("click", () => {
    setState({ network: "devnet", nodeUrl: "https://next.devnet.aztec-labs.com/" });
    chrome.runtime.sendMessage({ type: "SET_NETWORK", nodeUrl: "https://next.devnet.aztec-labs.com/", network: "devnet" });
    showToast("🔄 Aztec Devnet'e bağlandı", "success");
  });
}
    ...
    <div class="header">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        <div class="header-network" id="btn-network-toggle">
          ...
          ${store.network === "devnet" ? "Devnet" : "Sandbox"}
        </div>
  ...
  for (let y = 0; y < cells; y++) {
    for (let x = 0; x < cells; x++) {
      ...
      if (isCornerBorder || isCornerInner || isData) {
        rects += `<rect x="${x * cellSize}" y="${y * cellSize}" width="${cellSize - 1}" height="${cellSize - 1}" fill="#1a1a2e" rx="1"/>`;
      }
    }
  }
